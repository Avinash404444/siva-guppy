import {parcel,intatagramReelLink,fishlist,plant,bettaFishList,cactusPlantList} from "../data-base/data-file.js"

document.querySelector(".cart-bottom-btn").addEventListener("click",()=>location.href="#cart-section")
document.querySelector(".instagram-reel-link").src=intatagramReelLink+"/embed"

const profile=JSON.parse(localStorage.getItem("profile"))

if(profile){
    messagePopUp("login done surcesfully")
}else{
    messagePopUp("wellcome to siv guppy farm")
}
...
/* local storage cart remove section */
...
for(let j=0;j<toatalList.length;j++){
        ...
        let product=`<div class="product product-items-${toatalList[j][i][`id`]}">
                    ...
                    <div class="stock-info"><p>${toatalList[j][i]["stock"]=="y"?"in stock":"out of stock"}</p></div>
...
window.totalAmount=(value)=>{
    ...
    if(firstValue=="p"){
        ...
        if(product["stock"]=='y'){
            ...
            messagePopUp("add to cart done!")
        }
        else{
            messagePopUp("product is out of stock")
        }
...
window.productRemovercart=(value)=>{
    ...
    if(firstValue=="p"){
        ...
        const product=fishlist[remaingValue]
        product["totalPrice"]=0
        console.log(product);
        document.querySelector(`.input-${value}`).value=product["quantity"]
    }
...
function messagePopUp(value){
    ...
    document.querySelector(".message-box p").textContent=value
...
parcel.forEach((a) => {
  const s = `<img class="slide" src="../image/parcel/${a}">`;
  console.log(s);
  slideImg += s;
});
const sb = `
...
document.querySelector(".slider").innerHTML = slideImg;
