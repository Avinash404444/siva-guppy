...
const teldata=cartdata.map((i)=>{
    const data=`Product Name: ${i.name}\nNumber of quantity : ${i.quantity}\nEach product total anmount: ${i.quantity*i.totalPrice} \n\n`
    tpd+=data
})
...
const message =`New Order :\n\nCustomer Data:\n\n${Object.entries(profile).map(([key, value]) => `${key}: ${value}`).join("\n")}\n\nProduct Data:\n\n${tpd}\n Time: ${new Date().toLocaleString()}`;
    ...
    let url =`https://wa.me/916383970087?text=${encodeURIComponent(message)}`;
    window.open(url);
